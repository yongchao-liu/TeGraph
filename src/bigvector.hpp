...
public:
	...
	Vertices(std::string path, size_t length) {
		init(path, length);
	}
	Vertices(std::string path) {
		init(path);
	}
	~Vertices() {
	...
	void fill(const T & value) {
		...
		#pragma omp parallel num_threads(parallelism)
		{
			...
			std::tie(begin_i, end_i) = get_partition_range_exts(length, omp_get_num_threads(), omp_get_thread_num());
